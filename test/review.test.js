...
function createContext(reviewers) {
  return { config: jest.fn(path => path === 'bot-files/reviewers.yml' ? reviewers : undefined), github: { pulls: { createReviewRequest: jest.fn(param => 'toreview') } }, issue: jest.fn(param => 'issue'), payload: { pull_request: { user: { login: 'login' } } } }
}
